...
export default function EditPropertyPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchProperty = async () => {
    try {
      ...
      const propertyVideos = property.videos?.map((vid: any) => ({
      ...
      setVideos(propertyVideos)
      setFeatures(property.features?.map((f: any) => f.name || f) || [])
      setTags(property.tags?.map((t: any) => t.name || t) || [])
    } catch (error) {
          ...
          <div className="flex items-center justify-between py-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-gray-600">Update property listing</p>
              </div>
            </div>
          </div>
        </div>
